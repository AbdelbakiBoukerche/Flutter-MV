...
class _FTableViewState extends State<FTableView> {
  ...
  void initState() {
    ...
    model.addListener(_handleDataChanged);
    _handleDataChanged();
  }

  @override
  void didUpdateWidget(covariant FTableView oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (oldWidget.model != model) {
      oldWidget.model.removeListener(_handleDataChanged);
      model.addListener(_handleDataChanged);
      _handleDataChanged();
    }
  }
  ...
  @override
  void dispose() {
    model.removeListener(_handleDataChanged);
  ...
  void _handleLast() {
    pageTo(((model.rowCount() - 1) / widget.rowsPerPage).floor() *
        widget.rowsPerPage);
  }

  bool _isNextPageUnavailable() =>
      _firstRowIndex + widget.rowsPerPage >= model.rowCount();
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.onRowsPerPageChanged != null) {
      final List<Widget> availableRowsPerPage = widget.availableRowsPerPage
          .where((int value) =>
              value <= model.rowCount() || value == widget.rowsPerPage)
  ...
  List<DataRow> _getRows(int firstRowIndex, int rowsPerPage) {
    ...
    for (int index = firstRowIndex; index < nextPageFirstRowIndex; index += 1) {
      ...
      if (index < model.rowCount()) {
        row = _rows.putIfAbsent(index, () => _getRow(index));
  ...
  DataRow _getProgressIndicatorRowFor(int index) {
    bool haveProgressIndicator = false;
    final List<DataCell> cells = _columns.map<DataCell>((DataColumn column) {
      if (!column.numeric) {
        haveProgressIndicator = true;
